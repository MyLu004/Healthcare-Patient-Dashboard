...
# Set this in your backend .env / env vars
# e.g. VAPI_WEBHOOK_SECRET="super-secret"
...
async def vapi_tool_calls(
    ...
    """
    Vapi will POST tool-calls here.
    Body shape (simplified):
    {
      "message": {
        "type": "tool-calls",
        "toolCallList": [
          {"id": "toolCallId", "name": "list_availability", "arguments": {...}},
          ...
        ]
      }
    }
    ...
    if message.get("type") != "tool-calls":
        # ignore other Vapi message types
        return {"results": []}
    ...
    for tc in message.get("toolCallList", []) or []:
        ...
        # Some Vapi providers send arguments as JSON strings
        ...
        try:
                ...
                slots = [
                    {
                        ...
                        "visit_type": r.visit_type.value,  # VisitType enum
            ...
            # --------------------------
                ...
                visit_type = models.VisitType(args.get("visit_type", "telehealth"))

                facility_id = args.get("facility_id")
                if facility_id is not None:
                    facility_id = int(facility_id)

                # Business rules mirrored from your /appointments router
                if end_at <= start_at:
            ...
            # Never leak stack traces/PHI; return concise error
            ok({"error": str(e)})
